...
### 9.2 Option alternative — dump/restore de la base réelle (testée le 2026-07-15)
...
**Piège découvert en testant** : un dump *schéma + données* complet **ne
fonctionne pas** entre PostgreSQL 18 (local) et PostgreSQL 9.6 (O2switch) —
au-delà des soucis attendus (`\restrict`/`\unrestrict`, nouveaux paramètres
`SET`, extension `vector`), **chaque table utilise `GENERATED BY DEFAULT AS
IDENTITY`** pour ses clés primaires, une syntaxe PostgreSQL 10+ totalement
absente de 9.6. Un dump complet est donc irrécupérable tel quel sur O2switch.

**La méthode qui fonctionne** : ne jamais transférer le schéma via `pg_dump` —
laisser Django le générer nativement (il détecte la version PostgreSQL connectée
et utilise `SERIAL` au lieu d'`IDENTITY` quand nécessaire), et ne transférer que
les **données**.

1. **Export local — data-only, sans les tables gérées par Django lui-même**
   (`django_migrations`, `django_content_type`, `auth_permission`,
   `django_admin_log`, `django_session` — ces tables doivent refléter l'état du
   *serveur cible*, pas celui du poste local) :

   ```powershell
   pg_dump -U postgres -h localhost -p 5432 `
     --data-only --no-owner --no-privileges --disable-triggers --format=plain `
     ...
     -f dump_data_o2switch.sql recrutement_db
   ```

2. **Nettoyer les 3 lignes encore incompatibles** (toujours présentes même en
   data-only) : la ligne `\restrict ...` (début de fichier), la ligne
   `\unrestrict ...` (fin de fichier), et `SET transaction_timeout = 0;`.

3. **Générer un script de purge** pour vider, sur le serveur cible et juste
   après `migrate`, les tables que les migrations `pages`/`contenu`/etc. ont
   déjà pré-remplies via `RunPython` (sinon conflit de clé primaire à l'import) :
   `TRUNCATE TABLE public."<table>" RESTART IDENTITY CASCADE;` pour chaque table
   présente dans le dump (repérable via les lignes `COPY public.<table>` du
   fichier).

4. Transférer les 2 fichiers `.sql` sur O2switch — **pas via Git** (trop
   volumineux, pas leur place dans l'historique) — via le **Gestionnaire de
   fichiers** cPanel ou un client FTP/SFTP.

5. **Import, dans cet ordre précis**, depuis le Terminal cPanel :

   ```bash
   python manage.py migrate                                    # schéma natif PG9.6 (SERIAL)
   psql -U <user_pg> -h localhost <base> < 00_truncate_avant_import.sql
   psql -U <user_pg> -h localhost <base> < dump_data_o2switch_clean.sql
   ```

Les séquences (compteurs auto-incrémentés) sont resynchronisées automatiquement
par le dump (`pg_catalog.setval(...)` inclus pour chaque table) — pas de
commande supplémentaire après l'import.
